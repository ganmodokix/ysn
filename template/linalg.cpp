...
struct matrix {
    size_t rows, cols;
    vector<T> values;
    matrix(): rows(1), cols(1), values(1, (T)0) {}
    matrix(size_t rows_ = 1, size_t cols_ = 1, T default_value = 0):
    ...
    template <typename U>
    matrix(initializer_list<initializer_list<U>> init) : rows(init.size()), cols(init.begin()->size()), values(rows * cols, (T)0) { size_t i = 0, j = 0; for (const auto &initi : init) { for (const auto &initij : initi) { at(i, j) = initij; j++; } i++; j = 0; } }
    ...
    matrix<T> operator* (const matrix<T> &t) const { assert(cols == t.rows); matrix<T> r(rows, t.cols, (T)0); REP(k, cols) REP(i, rows) REP(j, t.cols) r.at(i, j) = add(r.at(i, j), mul(at(i, k), t.at(k, j))); return r; }
    template <typename S>
    ...
    static matrix<T> eye(size_t n) { matrix<T> r(n, n); REP(i, n) r.at(i, i) = 1; return r; }
    matrix<T> pow(ull n) const { assert(rows == cols); matrix<T> m = eye(rows), b = *this; while (n) { if (n & 1) m *= b; n >>= 1; b *= b; } return m; }
    matrix<T> transpose() const { matrix<T> r(cols, rows); REP(i, rows) REP(j, cols) r.at(i, j) = at(j, i); return r; }
};